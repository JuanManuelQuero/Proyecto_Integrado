<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Añada la seguridad adicional a su cuenta mediante la autenticación de dos pasos.') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Ha habilitado la autenticación de dos pasos.') }}
                @else
                    {{ __('No ha habilitado la autenticación de dos pasos.') }}
                @endif
            @else
                {{ __('No ha habilitado la autenticación de dos pasos.') }}
            @endif
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Cuando la autenticación de dos pasos está habilitada, se le solicitará un token seguro y aleatorio durante la autenticación. Puede recuperar este token de la aplicación Google Authenticator de su teléfono.') }}
            </p>
        </div>
        ...
        @if ($this->enabled)
            ...
            @if ($showingRecoveryCodes)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __('Guarde estos códigos de recuperación en un administrador de contraseñas seguro. Se pueden usar para recuperar el acceso a su cuenta si se pierde su dispositivo de autenticación de dos pasos.') }}
                    </p>
                </div>
